...
in rec {
    transformer = { config, ... }@all:
        ...
        l.foldl' (r: f: f r) res (if res.kind == "CustomResourceDefinition" then [
            ...
            (i: if path != null then utils.defaultAnnotation i "fractal.k8s.arctarus.net/flux-path" (l.concatStringsSep "/" path) else i)
        ])
    else res;
    ...
    in l.attrByPath [type] (throw "Type ${type} has no default Flux layer!") mapper;
    ...
    buildLayerKustomizations = config: let
        build = let
            inherit (config) cluster;
            sourceRef = cluster.renderer.flux.source;
        in layer: prev: l.recursiveUpdate
            {
                apiVersion = "kustomize.toolkit.fluxcd.io/v1beta1";
                kind = "Kustomization";
                metadata = {
    ...
    in l.imap0 (i: v: let
        prev = if i > 0 then l.elemAt layers (i - 1) else null;
